...
public:
    StartInterface() : speed(35) {
        // 开场动画
        startsnake.emplace_back(Point(0, 14));// 蛇，长10高5
        ...
        // 标题snake
        textsnake.emplace_back(Point(-25, 14));//S
        ...
        // 测试符号
        testpoint.emplace_back(Point(1, 1));
    }
    void PrintTest();// 测试用
    void PrintFirst();// 蛇
    void PrintSecond();// 移蛇
    void PrintThird();// SNAKE
    void PrintInfo();// 个人信息
    void PrintText();// 打印框内文字
    void ClearText();// 清冗余右平移
    void StartAction();//开始动画
private:
    std::deque<Point> startsnake;// 开始动画中的蛇
    std::vector<Point> textsnake;// 开始动画中的文字
    std::vector<Point> testpoint;// test
    int speed;//动画的速度
};
